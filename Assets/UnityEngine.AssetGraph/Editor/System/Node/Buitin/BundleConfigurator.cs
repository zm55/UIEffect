	...
	public class BundleConfigurator : Node, Model.NodeDataImporter {
		...
		public override void OnInspectorGUI(NodeGUI node, AssetReferenceStreamManager streamManager, NodeGUIEditor editor, Action onValueChanged) {
			...
			using (new EditorGUILayout.VerticalScope(GUI.skin.box)) {
				var newSetBundleNameValue = GUILayout.Toggle(m_setBundleNameAndVariant, "Set Bundle Name And Variant");
				if(newSetBundleNameValue != m_setBundleNameAndVariant) {
					using(new RecordUndoScope("Change Bundle Config", node, true)){
						m_setBundleNameAndVariant = newSetBundleNameValue;
					}
				}
		...
		{
			if (incoming == null || !m_setBundleNameAndVariant)
				return;
