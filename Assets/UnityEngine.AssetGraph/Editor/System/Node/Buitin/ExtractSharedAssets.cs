...
namespace UnityEngine.AssetGraph {
    ...
    public class ExtractSharedAssets : Node {
		...
		[SerializeField] private bool m_setBundleName;
    	...
    	public override void OnInspectorGUI(NodeGUI node, AssetReferenceStreamManager streamManager, NodeGUIEditor editor, Action onValueChanged) {
            ...
            using (new EditorGUILayout.VerticalScope(GUI.skin.box)) {
				var newSetBundleNameValue = EditorGUILayout.ToggleLeft("Set Bundle Name", m_setBundleName);
				if(newSetBundleNameValue != m_setBundleName) {
					using(new RecordUndoScope("Change Bundle Config", node, true)){
						m_setBundleName = newSetBundleNameValue;
					}
				}
		...
		{
			if (incoming == null || !m_setBundleName)
				return;
			...
			if (sharedDependency.Keys.Count > 0) {
				...
				foreach (var bundleName in sharedDependency.Keys) {
					...
					foreach (var a in sharedDependency [bundleName]) {
						var importer = AssetImporter.GetAtPath (a.path);
						if (importer != null) {
